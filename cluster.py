...
def Cluster(sim, th):
    ...
    ind = np.where(SR_temp > 1)[0].tolist()  # A point associated with multiple cluster centers.
    sim_core = sim[indices]
    max_indices = np.argmax(sim_core.T[ind], axis=1)  # The index of the largest cluster corresponding to a point associated with multiple cluster centers.
    cluster = []
    i = 0
    while i < len(indices):
        cluster1 = []
        neigh = np.where(SR_old[indices[i]] == 1)[0]  # The index of the neighbor corresponding to the ith cluster.
...
def has_duplicates(lst):
    flat_lst = np.concatenate(lst)
    unique_lst = np.unique(flat_lst)
    return len(unique_lst) != len(flat_lst)
