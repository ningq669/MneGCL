...
def keep_only_digits(arr):
    # 定义正则表达式模式，用于匹配数字
    pattern = r'\d+'
    # 创建一个向量化的函数，用于在每个字符串中查找匹配的数字部分
    vfunc = np.vectorize(lambda x: ''.join(re.findall(pattern, x)))
    # 应用向量化函数到NumPy数组中的每个元素
    result = np.apply_along_axis(vfunc, 1, arr)
    return result
