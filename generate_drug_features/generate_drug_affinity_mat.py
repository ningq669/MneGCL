...
def A_mat_update(X_mat, mu1, mu2, mu3, mu4, C1_mat, C2_mat, C3_mat, lambda1_mat, lambda2_mat, lambda3_mat, lambda4_mat):
    ...
    behind_mat = mu1 * K + (mu2) * (C2_mat - lambda2_mat / mu2) + (mu3) * (C1_mat - lambda3_mat / mu3) + (mu4) * (
            C3_mat - lambda4_mat / mu4) + np.dot(X_mat.T, lambda1_mat)
...
if __name__ == "__main__":
    ...
    while (iter < 100) and (not converged):
        ...
        for i in range(nv):
            A_prev[i] = A[i]
            A[i] = A_mat_update(X[i], mu1, mu2, mu3, mu4, C1[i], C2[i], C3[i], lambda1[i], lambda2[i], lambda3[i],
                                lambda4[i])
